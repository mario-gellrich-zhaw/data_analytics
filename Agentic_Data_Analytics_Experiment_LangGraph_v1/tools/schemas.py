...
WRITE_SCRAPER_CODE_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "canton field (state) is usually empty, so pre-filter by zipcode "
            "8000-8999 — only a rough cut (it also covers places in SZ, SG, TG, AG "
            "and SH): the exact canton filter happens in cleaning, by coordinates, "
            "so always save latitude/longitude. Catch ScrapeBlocked per "
...
WRITE_PREP_CODE_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "— e.g. layer "
            "ch.swisstopo.swissboundaries3d-kanton-flaeche.fill (the canton a point "
            "lies in: one result, attribute 'ak', e.g. 'ZH'), "
            ...
            f"drop more than {prep_code.MAX_DROPPED_SHARE:.0%} of the rows, and when "
            "the listings have coordinates it must look up each one's canton, save "
            f"it as '{prep_code.CANTON_COLUMN}' (the one new column cleaning may add) "
            f"and keep only '{prep_code.TARGET_CANTON}'; "
