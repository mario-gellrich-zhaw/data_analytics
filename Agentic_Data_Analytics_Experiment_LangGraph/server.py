...
def _render_conversation_history(
    ...
    lines = [
        "# Agentic Conversation — Data Analytics Process Model (LangGraph)",
...
def _run_demo(q: "queue.Queue"):
    ...
    try:
        ...
        # --- Agents ----------------------------------------------------------
        # Each agent is a LangChain ChatOpenAI (`.bind_tools(...)`-ed when
        # tools apply for the current phase). Tool EXECUTION is looked up
        ...
        phase_graph = build_phase_graph()  # one compiled graph, reused for every phase below
        ...
        def run_phase(
            ...
            for state in phase_graph.stream(initial_state, stream_mode="values"):
                final_state = state
                if state["turns_in_phase"] > emitted and state["last_turn"]:
                    turn = state["last_turn"]
                    stream_turn(turn["speaker"], turn["text"], turn["used_tool"], step, step_label)
